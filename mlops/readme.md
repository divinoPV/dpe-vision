...
```bash
docker compose exec dpe_vision-mlops-service /bin/bash -c "python src/experiment_tracking.py"
```

# Run MLFlow Server 
```bash
docker compose exec dpe_vision-mlops-service /bin/bash -c "mlflow server --host 0.0.0.0 --port 8080"
```
